...
class SeqSimulatedTVLighting : public Sequence
{
private:
	...
	int16_t pos;
	boolean varyRate;  // used to switch things on/off
	boolean varyBrightness;
	boolean varySaturation;

    void secondTick() {}

public:
	SeqSimulatedTVLighting(struct CRGB* data, int nLedsOrOffset) : Sequence(data, nLedsOrOffset) {}
    ...
    void DrawOneFrame(byte startHue8, int8_t yHueDelta8, int8_t xHueDelta8)
    {
        ...
        for (byte y = 0; y < kMatrixHeight; y++)
        {
            ...
            for (byte x = 0; x < kMatrixWidth; x++)
            {
                ...
                LEDArray[pos] = CHSV(pixelHue, sat, valX);

                if (varySaturation)
                {
                    CRGB color = LEDArray[pos];
                    ...
                    LEDArray[pos] = blend(color, mono, satMix);
                }
